    ...
    return (
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg p-4">
        <div className="flex items-center gap-2 text-yellow-700 dark:text-yellow-400">
          ...
          <span className="font-medium">{t('migration.devMode')}</span>
        </div>
        <p className="text-sm text-yellow-700 dark:text-yellow-500 mt-2">
          {t('migration.devModeDesc')}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
        <h4 className="font-medium text-gray-900 dark:text-white mb-2">{t('migration.localStorage')}</h4>
        {hasLocalData ? (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
            <div className="bg-white dark:bg-gray-700 rounded p-2 text-center">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">{t('migration.expensesLabel')}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 rounded p-2 text-center">
              <div className="text-lg font-semibold text-gray-900 dark:text-white">
                {localData?.income.length || 0}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">{t('migration.incomeLabel')}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 rounded p-2 text-center">
              <div className="text-lg font-semibold text-gray-900 dark:text-white">
                {localData?.accounts.length || 0}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">{t('migration.accountsLabel')}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 rounded p-2 text-center">
              <div className="text-lg font-semibold text-gray-900 dark:text-white">
                {localData?.ledgerAccounts.length || 0}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">{t('migration.ledgerLabel')}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 rounded p-2 text-center">
              <div className="text-lg font-semibold text-gray-900 dark:text-white">
                {localData?.snapshots.length || 0}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">{t('migration.snapshotsLabel')}</div>
            </div>
          </div>
        ) : (
          <p className="text-sm text-gray-500 dark:text-gray-400">{t('migration.noLocalData')}</p>
        )}
      </div>
      ...
      {supabaseData && (
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <h4 className="font-medium text-blue-900 dark:text-blue-100">{t('migration.cloudData')}</h4>
            <button
          ...
          {supabaseData.hasData ? (
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
              <div className="bg-white/50 dark:bg-blue-800/30 rounded p-2 text-center">
                ...
                <div className="text-xs text-blue-600 dark:text-blue-300">{t('migration.expensesLabel')}</div>
              </div>
              <div className="bg-white/50 dark:bg-blue-800/30 rounded p-2 text-center">
                <div className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                  {supabaseData.counts.income}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-300">{t('migration.incomeLabel')}</div>
              </div>
              <div className="bg-white/50 dark:bg-blue-800/30 rounded p-2 text-center">
                <div className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                  {supabaseData.counts.accounts}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-300">{t('migration.accountsLabel')}</div>
              </div>
              <div className="bg-white/50 dark:bg-blue-800/30 rounded p-2 text-center">
                <div className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                  {supabaseData.counts.ledgerAccounts}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-300">{t('migration.ledgerLabel')}</div>
              </div>
              <div className="bg-white/50 dark:bg-blue-800/30 rounded p-2 text-center">
                <div className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                  {supabaseData.counts.snapshots}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-300">{t('migration.snapshotsLabel')}</div>
              </div>
            </div>
          ) : (
            <p className="text-sm text-blue-600 dark:text-blue-300">{t('migration.noCloudData')}</p>
          )}
        </div>
      )}
        ...
        <div className={`rounded-lg p-4 ${
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`font-medium ${
              ...
              {migrationResult.success ? t('migration.success') : t('migration.errors')}
            </span>
          </div>
      ...
      {hasLocalData && (
        <div className="space-y-3">
          ...
          >
            ...
            {supabaseData?.hasData && overwrite ? t('migration.migrateOverwrite') : t('migration.migrateToCloud')}
          </button>
              ...
              {showClearConfirm ? (
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {t('migration.clearPrompt')}
                  </span>
                  <button
                    onClick={handleClearLocalData}
                    className="px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700"
                  >
                    {t('migration.clearConfirm')}
                  </button>
                  <button
